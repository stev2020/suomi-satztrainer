-- Reports no longer hide content for everyone at once.
-- Before: one report (up to 20 per account and hour) set a sentence or translation to
-- 'quarantined' for all visitors, so a few accounts could empty the corpus.
-- Now: the reporter hides it locally right away; global quarantine needs a reviewer
-- or 3 distinct reporters whose accounts are at least 24 hours old. Every report
-- still lands in the review queue ('needs_review'). Disabled content stays disabled.
create or replace function quality_private.report_quarantine_threshold()
returns integer language sql immutable set search_path='' as $$ select 3 $$;
create or replace function quality_private.report_min_account_age()
returns interval language sql immutable set search_path='' as $$ select interval '24 hours' $$;
revoke all on function quality_private.report_quarantine_threshold() from public,anon,authenticated;
revoke all on function quality_private.report_min_account_age() from public,anon,authenticated;
...
declare
 ...
 n integer; teacher boolean; room classroom_private.rooms; result jsonb; quarantine boolean;
begin
 ...
 if action='report' then
   ...
   -- A single report hides the content only for the reporter (the browser does that
   -- locally). Everyone else keeps seeing it until a reviewer reports it, a reviewer
   -- decides in the review queue, or enough independent, established accounts agree.
   select count(distinct r.reporter_id) into n
     from quality_private.reports r join auth.users u on u.id=r.reporter_id
    where r.sentence_id=sid and r.status='open'
      and (case when report_category='translation' and tid>0 then r.category='translation' and r.translation_id=tid
                else r.category<>'translation' end)
      and u.created_at<=now()-quality_private.report_min_account_age();
   quarantine:=exists(select 1 from quality_private.reviewers v where v.user_id=uid)
     or n>=quality_private.report_quarantine_threshold();
   ...
   if report_category='translation' and tid>0 then
     ...
     values(sid,tid,report_source_kind,'needs_review',case when quarantine then 'quarantined' else 'active' end)
     on conflict(sentence_id,translation_id) do update set
       review_status=case when quality_private.translation_states.availability='disabled' then quality_private.translation_states.review_status else 'needs_review' end,
       availability=case when quality_private.translation_states.availability='disabled' then 'disabled'
                         when quarantine then 'quarantined' else quality_private.translation_states.availability end,
       updated_at=now();
     return jsonb_build_object('quarantined',quarantine,'target','translation');
   else
     insert into quality_private.sentence_states(sentence_id,source_kind,review_status,availability)
     values(sid,report_source_kind,'needs_review',case when quarantine then 'quarantined' else 'active' end)
     on conflict(sentence_id) do update set
       review_status=case when quality_private.sentence_states.availability='disabled' then quality_private.sentence_states.review_status else 'needs_review' end,
       availability=case when quality_private.sentence_states.availability='disabled' then 'disabled'
                         when quarantine then 'quarantined' else quality_private.sentence_states.availability end,
       updated_at=now();
     return jsonb_build_object('quarantined',quarantine,'target','sentence');
   end if;
 end if;
