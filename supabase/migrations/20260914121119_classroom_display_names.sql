-- Names belong to a room, never to the login profile. Retain names after leaving
-- so existing contributions do not revert to a login name.
create table classroom_private.display_names (
...
create index display_names_user_id_idx on classroom_private.display_names(user_id);
alter table classroom_private.display_names enable row level security;
create policy "deny direct browser access" on classroom_private.display_names
 for all to anon, authenticated using(false) with check(false);
revoke all on classroom_private.display_names from public,anon,authenticated;

-- Preserve existing names, including authors who have already left a room.
insert into classroom_private.display_names(room_id,user_id,name)
select distinct x.room_id,x.user_id,p.username from (
 select id room_id,owner_id user_id from classroom_private.rooms
 union select room_id,user_id from classroom_private.members
 union select room_id,user_id from classroom_private.stream_posts
 union select a.room_id,s.user_id from classroom_private.submissions s join classroom_private.assignments a on a.id=s.assignment_id
 union select a.room_id,m.user_id from classroom_private.messages m join classroom_private.assignments a on a.id=m.assignment_id
) x join public.profiles p on p.user_id=x.user_id;

-- Only used by the existing, authenticated and membership-checked private APIs.
create function classroom_private.display_name(rid uuid,uid uuid)
returns text language sql stable security invoker set search_path='' as $$
 select coalesce((select name from classroom_private.display_names where room_id=rid and user_id=uid),'Ehemaliges Mitglied');
$$;
revoke all on function classroom_private.display_name(uuid,uuid) from public,anon,authenticated;
 ...
 if action='stream_list' then
  ...
  return jsonb_build_object(
   'posts',(select coalesce(jsonb_agg(to_jsonb(q) order by q.pinned desc,q.created_at desc,q.id),'[]') from (
    select s.id,s.kind,s.body,s.resolved,s.pinned,s.deleted,s.created_at,classroom_private.display_name(rid,s.user_id) as author,s.user_id=uid as own,classroom_private.is_teacher(rid,s.user_id) as teacher,
