...
function persist(){memory.prefs={level,direction,audioOnly,activity,speed,grammarTopic};try{localStorage.setItem(STORE,JSON.stringify(memory));}catch{$('notice').textContent='Dein Browser kann den Lernstand gerade nicht speichern. Er bleibt für diese Sitzung erhalten.';}}
...
function startWritingSession(force=false,reviewOnly=false){
 if(force||!writingSessions[level]?.items.length)writingSessions[level]={items:(reviewOnly?writingReviewPool():shuffle(writingPool())).slice(0,10),answers:[],draft:'',reviewOnly,ratings:{}};
}
function rateWritingAnswer(session,id,rating){
 ...
 renderWritingSession();$('notice').textContent=rating==='right'?'Als richtig gespeichert. Dieser Satz steht nicht mehr auf deiner Wiederholungsliste.':'Für die gezielte Wiederholung gespeichert.';
...
function renderWritingSession(){
 ...
 const session=writingSessions[level];if(!session?.items.length){$('card').className='card empty';$('card').innerHTML='<h2>Noch keine Sätze bereit.</h2><p>Übe einen Satz zuerst und wiederhole ihn mindestens zweimal in derselben Lernrichtung oder Hörübung. Du kannst auch ein anderes Level wählen.</p>';$('actions').innerHTML='<button id="writing-go-learn" class="primary">Sätze üben</button>';$('writing-go-learn').onclick=()=>{activity='translate';mode='new';start();};return;}
 ...
 if(count===total){
  ...
  $('actions').innerHTML='<button class="primary" id="writing-new-session">Neuer Schreibtest</button>';$('writing-new-session').onclick=()=>{startWritingSession(true);$('notice').textContent='';render();$('writing-input')?.focus();};
...
function grade(g){if(activity==='writing')return;if(!revealed||!queue.length)return;const s=queue.shift(),interval=g==='again'?0:g==='hard'?1:easyDays(s);memory.reviews[key(s)]={due:Date.now()+interval*86400000,interval,repetitions:(Number(review(s)?.repetitions)||0)+1,updatedAt:Date.now()};memory.daily[day()]=(Number(memory.daily[day()])||0)+1;if(g==='again')queue.splice(Math.min(2,queue.length),0,s);completed++;revealed=false;draft='';persist();render();$('reveal')?.focus({preventScroll:true});}
async function play(a,restart=false){const b=$('play-audio');if(player&&!player.paused&&!restart){stopAudio();b.querySelector('span').textContent='Anhören';return;}stopAudio();player=new Audio(a.download_url);player.playbackRate=speed;player.preservesPitch=true;const current=player;b.querySelector('span').textContent='Lädt …';const reset=()=>{if(player===current&&$('play-audio')===b)b.querySelector('span').textContent='Anhören';};current.onended=reset;current.onerror=()=>{if(player!==current)return;reset();$('notice').textContent='Die Aufnahme ist gerade nicht erreichbar. Versuche den Link „Audiodatei“ oder prüfe deine Internetverbindung.';};try{await current.play();if(player===current)b.querySelector('span').textContent='Anhalten';}catch{if(player!==current)return;reset();$('notice').textContent='Die Aufnahme konnte nicht abgespielt werden. Du kannst sie über „Audiodatei“ öffnen.';}}
...
function commitLearning(candidate){localStorage.setItem(STORE,JSON.stringify(candidate));memory=candidate;}
...
$('manage').onclick=()=>{stopAudio();clearImport();$('backup-status').textContent='';renderReportList();$('manage-dialog').showModal();};
