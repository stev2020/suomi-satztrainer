...
function dictationMarkup(s){
 ...
 return `<div class="dictation comparison"><h3>${c.same?'Richtig gehört und geschrieben!':'Vergleiche die markierten Stellen.'}</h3><span>Deine Eingabe</span><p lang="fi">${c.typed||'—'}</p>${c.same?'':`<span>Vorlage · fehlende oder andere Zeichen</span><p lang="fi">${c.expected}${sourceIcon(s)}</p>`}<small>Großschreibung, Satzzeichen und zusätzliche Leerzeichen werden beim Vergleich ignoriert.</small></div>`;
}
...
function wordPracticeMarkup(s){
 ...
 if(revealed){const correct=wordAnswerMatches(w),solution=language==='de'?s.translations[0]:s;return `<div class="word-result ${correct?'correct':'incorrect'}"><strong>${correct?'Richtig!':'Noch nicht ganz.'}</strong><p class="word-attempt" lang="${language}">${escape(draft)}</p>${correct?'':`<div class="word-correction"><span class="card-label">Richtige Lösung</span><p lang="${language}">${escape(solution.text)}${sourceIcon(solution)}</p></div>`}<small>${correct?'Perfekt zusammengesetzt.':'Mit „Nochmal“ übst du die richtige Wortfolge direkt erneut.'}</small></div>`;}
...
function render(){applyDailyCard();if(activity==='verbs'){renderVerbSession();return;}if(activity==='writing'){renderWritingSession();return;}$('writing-history').hidden=true;stopAudio();renderStats();preloadQueueAudio();const s=queue[0];$('actions').innerHTML='';$('keyboard-note').hidden=!s;$('keyboard-note').textContent=isTranslation()?'Nach dem Vergleich: 1 / 2 / 3 zum Bewerten':'Aufnahme beliebig oft anhören · Nach dem Aufdecken: 1 / 2 / 3 zum Bewerten';
 ...
 $('card').innerHTML=`<div class="card-top"><span class="card-label">${!isTranslation()?(activity==='listen'?'Hörmodus':'Diktat'):cardDirection(s)==='fi-de'?'Finnisch':'Deutsch'} <span>·</span> Level ${s.level}${s.level_assessment?.status==='estimated'?' · geschätzt':''}</span><button class="favorite ${saved?'saved':''}" id="favorite" aria-label="${saved?'Aus Favoriten entfernen':'Als Favorit speichern'}" aria-pressed="${saved}">${saved?'★':'☆'}</button></div>${questionMarkup(s,front)}${audioMarkup(s)}${dictationMarkup(s)}${translationDraftMarkup(s)}${revealed?`<div class="translation ${activity==='translate'&&!isWordPractice()?'hard-translation-solution':''}" ${isWordPractice()?'hidden':''}><span class="card-label" style="justify-content:center">${isTranslation()?'Vorlage · ':''}${!isTranslation()||cardDirection(s)==='fi-de'?'Deutsch':'Finnisch'}</span>${!isTranslation()||cardDirection(s)==='fi-de'?s.translations.map((t,i)=>`<p lang="de" class="${i?'variant':''}">${escape(t.text)}${sourceIcon(t)}</p>`).join(''):`<p lang="fi">${escape(s.text)}${sourceIcon(s)}</p>`}</div><div id="inline-grades" class="actions inline-grades" aria-label="Antwort bewerten"></div>${grammarMarkup(s)}${translationNote(s)?`<p class="bridge-note">${escape(translationNote(s))}</p>`:''}<details class="sources"><summary>Quellen &amp; Aufnahmen</summary><p>Finnisch: ${source(s)}</p>${s.translations.map(t=>`<p>Deutsch: ${source(t)}</p>`).join('')}${s.audios.map(a=>`<p>Aufnahme: <a href="${safeURL(a.attribution_url||`https://tatoeba.org/en/user/profile/${encodeURIComponent(a.author)}`)}" target="_blank" rel="noopener">${escape(a.author)}</a> · <a href="${safeURL(licenseURL(a.license))}" target="_blank" rel="noopener">${escape(a.license)}</a></p>`).join('')}</details>`:''}`;
 if(revealed){
