...
function renderRoom(){
 ...
 <div class="cr-hero cr-stream-hero"><div><span class="cr-badge">GEMEINSAM FINNISCH LERNEN${room.archived?' · ARCHIV':''}</span><h2>${esc(room.name)}</h2><p>${room.teacher_count} ${room.teacher_count===1?'Lehrkraft':'Lehrkräfte'} · ${room.member_count} Teilnehmer</p></div>${room.teacher&&!room.archived?b('Aufgabe erstellen','new_assignment'):''}</div>
 ...
 <section class="cr-card"><h3>Unsere Klasse</h3><details class="cr-card cr-roster" open><summary>Mitglieder · ${room.members.filter(m=>!m.blocked).length}</summary>${room.members.map(m=>`<div class="cr-member"><span>${esc(m.name)} · ${m.owner?'Lehrkraft · Ersteller':m.role==='teacher'?'Lehrkraft':'Teilnehmer'}${m.blocked?' · entfernt':''}</span>${room.owner&&!m.owner&&!m.blocked&&!room.archived?`<div class="cr-member-actions">${b(m.role==='teacher'?'Lehrkraftrolle entziehen':'Zur Lehrkraft machen','set_role',`data-id="${m.id}" data-role="${m.role==='teacher'?'student':'teacher'}"`)}${b('Entfernen','remove',`data-id="${m.id}"`)}</div>`:''}</div>`).join('')}</details>${room.owner&&!room.archived?'<p class="cr-note">Lehrkräfte können Aufgaben erstellen, Abgaben einsehen und den Austausch betreuen. Nur du als Ersteller verwaltest Rollen und den Raum.</p>':''} ${room.owner?`<details class="cr-card"><summary>Einladung &amp; Mitglieder verwalten</summary><p>Einladungscode: <strong class="cr-code">${esc(room.code)}</strong></p><div class="cr-toolbar">${b('Code kopieren','copy')}${!room.archived?b('Code erneuern','rotate')+b('Raum archivieren','archive'):''}${b('Klassenraum löschen','delete_room','data-danger="true"')}</div><form id="cr-delete-confirm" data-cr-form="delete_room" class="cr-delete-warning" hidden><h3>Klassenraum endgültig löschen</h3><p>Alle Aufgaben, Abgaben, Fragen, Reaktionen und Mitgliedschaften dieses Raums werden unwiderruflich gelöscht. Nutzerkonten und persönliche Lernstände bleiben erhalten.</p><label>Zur Bestätigung den Raumnamen „${esc(room.name)}“ eingeben<input name="confirm_name" required maxlength="80" autocomplete="off"></label><div class="cr-toolbar"><button class="quiet cr-danger" type="submit">Endgültig löschen</button>${b('Abbrechen','cancel_delete')}</div></form><p class="cr-note">Entfernte Mitglieder können mit diesem Konto nicht erneut beitreten. Archivierte Räume bleiben lesbar.</p></details>`:`<p class="cr-note">Deine Abgaben sehen die Lehrkräfte. Nach der Freigabe sieht die Klasse Antworten ohne Nutzernamen; dies ist keine Garantie gegen Wiedererkennung. Fragen erscheinen mit Nutzernamen.</p>${b('Raum verlassen','leave')}`}</section>
