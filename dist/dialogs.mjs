...
export function renderDialogs(ctx){
 ...
 card.innerHTML=`<div class="card-top"><span class="card-label">Dialog · Level ${dialog.level}</span><span>${Math.min(session.position,lines.length)} / ${lines.length}</span></div><h2 class="dialog-title">${escape(dialog.title)}</h2><p class="dialog-scene">${escape(dialog.scene)}</p><p class="dialog-cast">${roles.map(([,p])=>`<span><b>${escape(p.name)}</b> · ${escape(p.role)}</span>`).join('')}</p><ol class="dialog-lines">${visible.map((line,i)=>lineMarkup(session,line,i)).join('')}</ol>${finished&&dialog.phrases?.length?`<div class="dialog-phrases"><h3>Wendungen aus dem Dialog</h3>${dialog.phrases.map(p=>`<p><b lang="fi">${escape(p.fi)}</b> – ${escape(p.de)}</p>`).join('')}</div>`:''}`;
