...
import {mountSearch,searchInstructions} from './dist/wordsearch-ui.mjs';
...
function setup(){
 ...
 let completed=0;mountSearch(container,puzzle,()=>completed++,'Wo ist die Katze?');
 assert.equal(nodes.get('.search-prompt').textContent,'Wo ist die Katze?');
...
assert.match(searchInstructions({difficulty:'easy'}),/Finde die finnischen Wörter/);
assert.match(searchInstructions({difficulty:'hard'}),/diagonal – auch rückwärts/);
