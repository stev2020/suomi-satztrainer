...
for(let level=1;level<=6;level++){
 const topics=LEVEL_PATHS[level],ids=topics.flatMap(t=>t.lessons.flatMap(l=>l.ids));allIds.push(...ids);
 ...
 assert.equal(state.seen,0);assert.equal(state.total,ids.length);assert(!state.complete);
 ...
 state=everydayPathState(sentences,reviews,level);assert(state.complete);assert.equal(state.lesson,null);assert.equal(state.seen,ids.length);
 ...
 for(const id of ids)reviews[id+':fi-de'].due=Date.now()+86400000;
...
console.log(`PASS: six level paths, 13 topics each, ${allIds.length} distinct IDs, prior progress, missing assets, empty topics and level-scoped reviews.`);
