...
Vierte Übungsart `writing`: zufällig bis zu zehn verschiedene aktive/archivierte Sätze des gewählten Levels, jeweils Deutsch → Finnisch. Zulassung ausschließlich über vorhandene Lernstände: mindestens drei Bewertungen (`repetitions >= 3`) in wenigstens einem der bisherigen Pfade `fi-de`, `de-fi`, `listen`, `dictation`. Das entspricht erstem Lernen plus zwei Wiederholungen nach dem vorhandenen Zählmodell, einschließlich „Nochmal“. Bewertungen verschiedener Pfade werden nicht addiert. Audiofilter und normale Neue-/Wiederholen-/Favoriten-Auswahl gelten in diesem Modus nicht.
